document.getElementById('rephraseButton').addEventListener('click', async function () {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.candidates && data.candidates.length > 0 && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts.length > 0) {
          ...
          // If needed, escape special characters to render them correctly
          rephrasedSentence = escapeHtml(rephrasedSentence);
  ...
  // Function to escape special characters like <, >, &, etc.
